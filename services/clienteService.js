...
const update = async (cliente) => {
    ...
    const isOk = await (await connection)
        .execute(query, [cliente.nome, cliente.sobrenome, cliente.email, cliente.idade, cliente.id]);
...
const save = async (cliente) => {
    ...
    const isOk = await (await connection)
        .execute(query, [cliente.nome, cliente.sobrenome, cliente.email, cliente.idade]);
    return isOk[0].affectedRows === 1;
};
